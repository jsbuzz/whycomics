...
$(function() {
    ...
    $('.link-box').each(function() {
        ...
        .mouseenter(debounce(function() {
            ...
            $('.icon, .icon-bg', linkBox).stop().fadeIn().addClass('active');
        ...
        .mouseleave(function() {
            var linkBox = this;
            $('.icon, .icon-bg', linkBox).stop().fadeOut().removeClass('active');
    ...
    $(document).scroll(debounce(function() {
        $('.icon:not(.active)').fadeIn(200, function() {
            $('.icon:not(.active)').fadeOut(200);
        });
    }, 600))
    ...
    $(document).scroll(debounce(function() {
        ...
        // frame 12
        scrollStart = $('#comic-strip-12').position().top - $('#comic-strip-12').height()/3;
        if(scrollTop > scrollStart && scrollTop < scrollStart + 3000) {
            var unit = (scrollTop - scrollStart) / 8;
